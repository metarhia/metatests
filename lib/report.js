...
yamlCustomTags.find((t) => t.tag === '!error').identify = isError;
yamlCustomTags.find((t) => t.tag === '!date').identify = isDate;
yamlCustomTags.find((t) => t.tag === '!re').identify = isRegExp;
...
class TapReporter extends Reporter {
  constructor(options) {
    super(options);
    // TODO(lundibundi): fix (remove)
    // this once TSR properly supports require from worker_threads
