...
const getPath = () => {
  ...
  return obj.stack.split('(')[3].split(':')[0];
};
...
const speed = (
  ...
  anomalyPercent, // Number, max percent of anomaly
  testsSync, // Array of synchronous functions
  testsAsync = [], // Array of asynchronous functions, callback-last
  versions = [process.versions.node] // Array, node ver-s: [8.0.0, 9.4, 6]
) => {
  const path = getPath(); // path to file that contains tested functions
  ...
  const sendRequest = (
    ...
    const forked = fork(PATH_GET_TIME, [path], { execPath: nodePath });
    forked.send({ name: func.name, count, type, path, anomalyPercent });
