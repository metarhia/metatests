...
const getModRef = () => {
  let modRef = module;
  while (modRef.parent !== null) modRef = modRef.parent;
  return process.argv[1] === PATH_TO_CHILD ? modRef.children[0] : modRef;
};
...
const speed = (
  ...
  const path = modRef.filename; // string, path to file with test function
