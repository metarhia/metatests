...
//         count <number> number of times it will be run
//         args <Array> function arguments
//       done <Function> called when run in done, optional
//         name <string> case name
//         results <Object> case run results
...
//   count <string> number of times case was run
//   time <string> time in nanoseconds it took to make `count` runs
//   result <string> result of one of the runs
const measure = (
  cases,
  {
    defaultCount = 1e6,
  ...
  const res = [];
  cases.forEach(({ name, fn, argCases = [], n = defaultCount }) => {
    if (!n || n <= 0)
    ...
    for (const args of argCases) {
      for (let i = 0; i < runs; i++) {
        ...
        if (listener.done) listener.done(testName, result);
        res.push(result);
      }
    }
  });
...
const speed = (caption, count, cases) => {
  ...
  const ladder = times.sort((t1, t2) => t1.time - t2.time);
  const best = ladder[0].time;
  console.log(caption);
  ladder.forEach(test => {
    const name = test.name.padEnd(25, '.');
    const time = test.time.toString().padStart(15, '.');
    const percent = roundTo((test.time * 100) / best - 100, 2).toString();
    const percentstr = (percent > 0 ? `+${percent}%` : 'min').padStart(10, '.');
    console.log(name + time + percentstr);
  });
  console.log();
};
