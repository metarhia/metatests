...
const outputToConsole = (
  ...
  result.forEach((times, version) => {
    ...
    console.log(rpad('Name', 25) + lpad('Percent', 10) + lpad('Time', 15) +
      lpad('Anomaly', 10) + lpad('An.Time', 10));
    times.forEach(func => {
      const name = rpad(func.name, 25);
      const percent = lpad(func.percent, 10);
      const time = lpad((Math.round(func.time)).toString(), 15);
      const anomalyPercent = lpad(func.anomalyPercent.toString() + '%', 10);
      const anomalyTime = lpad(func.anomalyTime.toString() + '%', 10);
