...
class TimeCollector {
  ...
  getBufferSize(
    ...
    return Math.min(bufferSize, DEFAULT_BUFFER_SIZE);
  }
  ...
  sync(
    ...
    for (let iteration = 0; iteration < this.startCount; iteration++) {
      %OptimizeFunctionOnNextCall(fn); // eslint-disable-line
      this.intermediateValue.add(fn(), iteration);
    }
    for (let iteration = 0; iteration < this.count; iteration++) {
      const begin = process.hrtime();
      const result = fn();
      const end = process.hrtime(begin);
      fnEnvironment.add(end[0] * 1e9 + end[1], iteration);
      this.intermediateValue.add(iteration, result);
    }
  ...
  async(
    ...
    const testFunction = (begin, iteration) => fn((err, result) => {
      const end = process.hrtime(begin);
      this.intermediateValue.add(iteration, err, result);
      fnEnvironment.add(end[0] * 1e9 + end[1], iteration);
      if (++iteration < this.count) {
        testFunction(process.hrtime(), iteration);
        return;
      }
    ...
    const optimiseFunction = iteration => fn((err, result) => {
      this.intermediateValue.add(err, result, iteration);
      %OptimizeFunctionOnNextCall(fn); // eslint-disable-lines
      if (++iteration < this.startCount) optimiseFunction(iteration);
