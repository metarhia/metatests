...
function equal(val1, val2) {
  ...
  const array1 = Array.isArray(val1);
  const array2 = Array.isArray(val2);
...
function strictEqual(val1, val2) {
  ...
  const type2 = typeof(val2);
  if (type1 !== type2) return false;
  if (array1) return strictEqualArray(val1, val2);
...
module.exports = (metatests) => {
  metatests.equal = equal;
  metatests.strictEqual = strictEqual;
};
