...
const { isPromise, isError, transformToObject } = require('./utils');
...
class ImperativeTest extends Test {
  ...
  //   message <string> description of the check, optional
  error(err, message) {
    this._check('error', err => !isError(err), err, null, message);
  }
  ...
  contains(actual, subObj, message, sort, cmp = compare.strictEqual) {
    const success = compare.contains(sort, cmp)(actual, subObj);
    this._recordPass('contains', success, message, {
      expected: transformToObject(subObj, sort),
  ...
  //     expected <any>
  //     Returns: <boolean> true if actual is equal to expected, false otherwise
  containsGreedy(actual, subObj, message, cmp = compare.strictEqual) {
    const success = compare.containsGreedy(cmp)(actual, subObj);
  ...
  _end(error) {
    ...
    if (this.deferQueue.length > 0) {
      this._drainDeferQueue().finally(() => super._end(error));
    } else {
      super._end(error);
    }
  }
  ...
  //   fn <Function> function to call before the end of test.
  //       Can return a promise that will defer the end of test.
  //   options <Object>
  //     ignoreErrors <boolean> ignore errors from fn function, default: `false`
  defer(fn, { ignoreErrors = false } = {}) {
    this.deferQueue.push({ fn, ignoreErrors });
  }
  ...
  // #private
  async _drainDeferQueue() {
    for (const { fn, ignoreErrors } of this.deferQueue) {
      try {
        await fn(this);
      } catch (err) {
        if (ignoreErrors) {
          this.log('Defer callback errored', err);
        } else {
          const message = `Error in deferred callback: ${err.message}`;
          this._record('defer', false, message, {
            actual: err,
            stack: err.stack,
          });
        }
...
const test = (caption, func, options = {}, runner = runnerInstance) => {
  ...
  if (!runner.options.runTodo && opts.todo) {
    return null;
  }
  opts.runner = runner;
  const test = new ImperativeTest(caption, func, opts);
  runner.addTest(test);
  return test;
};
