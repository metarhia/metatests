...
const handleEachCallback = (func, test, callback) => {
  if (!func) {
    callback();
  } else if (func.length >= 2) {
    // eachFunc(test, callback)
    func(test, res => callback(null, res));
  } else {
    // eachFunc(test): Promise
    func(test).then(res => callback(null, res), err => callback(err));
  }
};
...
class ImperativeTest extends Test {
  ...
  _setupSubtestCallbacks() {
    ...
    this._subtestCallback = test => {
      ...
      handleEachCallback(this.afterEachFunc, test, err => {
        if (err) {
  ...
  [kRunSubtest](test) {
    ...
    handleEachCallback(this.beforeEachFunc, test, (err, context) => {
      if (err) {
  ...
  resolves(input, expected) {
    ...
    return input.then(
      res => {
        this.strictSame(res, expected);
        return res;
      },
      e => {
        this.fail('expected to be resolved, but was rejected', e);
        throw e;
      }
    );
  }
  ...
  rejects(input, err) {
    ...
    return input.then(
      res => {
        this.fail('expected to be rejected, but was resolved', res);
        throw res;
      },
      e => {
        this.isError(e, err);
